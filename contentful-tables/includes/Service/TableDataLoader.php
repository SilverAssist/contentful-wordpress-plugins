...
final class TableDataLoader implements LoadableInterface {
	 ...
	 */
	private array $tables_data = array();
	 ...
	 * @var array<string, array<string, mixed>>
	 */
	private array $charts_data = array();
	 ...
	 * @var array<string, array<string, mixed>>
	 */
	private array $cards_data = array();
			...
			foreach ( $csv_files as $file ) {
				...
				if ( false !== $csv_content ) {
					...
					if ( ! empty( $raw_data ) ) {
						...
						$this->tables_data[ $table_id ] = array(
							'type'           => 'Plain',
							'title'          => '',
							'style'          => 'Equal Width',
							'theme'          => 'Standard',
	...
	private function load_tables_from_post_meta(): void {
		$possible_post_ids = array( 241, 240, 238, 236, 235, 237, 239 );
