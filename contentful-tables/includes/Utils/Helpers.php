...
final class Helpers {
	...
	public static function format_header_label( string $header ): string {
		...
		$label = \str_replace( array( '_', '-' ), ' ', $header );
	...
	public static function resolve_title_placeholders( string $title ): string {
		...
		return \str_replace( array( '[ city-state ]', '[city-state]' ), $replacement, $title );
	}
	 ...
	 */
	public static function resolve_key_from_heading( string $heading_slug, array $key_values = array() ): ?string {
	 ...
	 */
	public static function filter_rows_by_key( array $rows, int $key_col_idx, string $filter, array $key_values = array() ): array {
